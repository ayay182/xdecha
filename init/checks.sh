...
_checkDefaultVars() {
    ...
    declare -rA def_vals=(
        ...
        [UPSTREAM_REPO]="https://github.com/ayay182/xdecha"
...
_checkUnoffPlugins() {
    ...
    if test $LOAD_UNOFFICIAL_PLUGINS = true; then
        ...
        gitClone --depth=1 https://github.com/ayay82/xdecha-Plugins.git
        editLastMessage "\t\tUpgrading PIP ..."
        upgradePip
        editLastMessage "\t\tInstalling Requirements ..."
        installReq xdecha-Plugins
        editLastMessage "\t\tCleaning ..."
        rm -rf xdecha/plugins/unofficial/
        mv xdecha-Plugins/plugins/ xdecha/plugins/unofficial/
        cp -r xdecha-Plugins/resources/* resources/
        rm -rf xdecha-Plugins/
        deleteLastMessage
