...
async def progress(current: int,
                   total: int,
                   message: 'xdecha.Message',
                   ud_type: str,
                   file_name: str = '',
                   delay: int = xdecha.Config.EDIT_SLEEP_TIMEOUT) -> None:
    """ progress function """
    ...
    if (now - last) >= delay:
        ...
        progress_str = progress_str.format(
            ud_type,
            file_name,
            ''.join((xdecha.Config.FINISHED_PROGRESS_STR
                     for i in range(floor(percentage / 5)))),
            ''.join((xdecha.Config.UNFINISHED_PROGRESS_STR
