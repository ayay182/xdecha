...
async def autopic(message: Message):
    ...
    if UPDATE_PIC:
        ...
        await xdecha.set_profile_photo(photo=BASE_PIC)
    ...
    if os.path.exists(BASE_PIC) and not image_path:
        ...
        profile_photo = await xdecha.get_profile_photos("me", limit=1)
        if not profile_photo:
            await message.err("sorry, couldn't find any picture!")
            return
        await xdecha.download_media(profile_photo[0], file_name=BASE_PIC)
        store = True
    else:
...
async def apic_worker():
    user_dict = await xdecha.get_user_dict('me')
    ...
    while UPDATE_PIC:
        if not count % Config.AUTOPIC_TIMEOUT:
            ...
            await xdecha.set_profile_photo(photo=MDFY_PIC)
            os.remove(MDFY_PIC)
