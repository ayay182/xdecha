...
async def kang_(message: Message):
    ...
    if replied and replied.media:
        ...
        photo = await xdecha.download_media(message=replied,
    ...
    if photo:
        ...
        packname = f"a{user.id}_by_xdecha_{pack}"
        ...
        if exist is not False:
            async with xdecha.conversation('Stickers', limit=30) as conv:
                try:
            ...
            async with xdecha.conversation('Stickers') as conv:
                try:
                    await conv.send_message(cmd)
                except YouBlockedUser:
                    await message.edit('first **unblock** @Stickers')
