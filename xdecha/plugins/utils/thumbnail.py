...
from xdecha import xdecha, Config, Message, get_collection
from xdecha.utils import progress
...
@xdecha.on_cmd('dthumb', about={'header': "Delete thumbnail"}, allow_channels=False)
async def clear_thumb_nail(message: Message):
    ...
    elif os.path.exists('resources/xdecha.png'):
        os.remove('resources/xdecha.png')
        await message.edit("✅ Default thumbnail deleted succesfully.", del_in=3)
    else:
        await message.delete()


@xdecha.on_cmd('vthumb', about={'header': "View thumbnail"}, allow_channels=False)
