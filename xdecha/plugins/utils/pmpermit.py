...
from xdecha import xdecha, filters, Message, Config, get_collection
from xdecha.utils import SafeDict
...
@xdecha.on_cmd("allow", about={
    'header': "allows someone to contact",
    'description': "Ones someone is allowed, "
                   "xdecha will not interfere or handle such private chats",
...
async def allow(message: Message):
    ...
    if userid:
        ...
        if a.matched_count:
            ...
            await (await xdecha.get_users(userid)).unblock()
...
async def get_id(message: Message):
    ...
    if message.input_str:
        ...
        try:
            userid = (await xdecha.get_users(user)).id
...
@xdecha.on_cmd("setpmmsg", about={
    'header': "Sets the reply message",
    'description': "You can change the default message which xdecha gives on un-invited PMs",
...
@xdecha.on_filters(~allowAllFilter & filters.incoming & filters.private & ~filters.bot
                   & ~filters.me & ~filters.service & ~Config.ALLOWED_CHATS, allow_via_bot=False)
async def uninvitedPmHandler(message: Message):
    """ pm message handler """
    user_dict = await xdecha.get_user_dict(message.from_user.id)
    ...
    if message.from_user.id in pmCounter:
        ...
        if xdecha.has_bot and _IS_INLINE:
            try:
                bot_username = (await xdecha.bot.get_me()).username
                k = await xdecha.get_inline_bot_results(bot_username, "pmpermit")
                await xdecha.send_inline_bot_result(
...
@xdecha.on_filters(~allowAllFilter & filters.outgoing & ~filters.edited
...
async def outgoing_auto_approve(message: Message):
    ...
    await ALLOWED_COLLECTION.update_one(
        {'_id': userID}, {"$set": {'status': 'allowed'}}, upsert=True)
    user_dict = await xdecha.get_user_dict(userID)
    await CHANNEL.log(f"**#AUTO_APPROVED**\n{user_dict['mention']}")

if xdecha.has_bot:
    @xdecha.bot.on_callback_query(filters.regex(pattern=r"pm_allow\((.+?)\)"))
    async def pm_callback_allow(_, c_q: CallbackQuery):
        owner = await xdecha.get_me()
        if c_q.from_user.id == owner.id:
            userID = int(c_q.matches[0].group(1))
            await xdecha.unblock_user(userID)
            user = await xdecha.get_users(userID)
            if userID in Config.ALLOWED_CHATS:
    ...
    @xdecha.bot.on_callback_query(filters.regex(pattern=r"pm_block\((.+?)\)"))
    async def pm_callback_block(_, c_q: CallbackQuery):
        ...
        if c_q.from_user.id == owner.id:
            ...
            await xdecha.block_user(userID)
            if userID in pmCounter:
                del pmCounter[userID]
            if userID in Config.ALLOWED_CHATS:
                Config.ALLOWED_CHATS.remove(userID)
    ...
    @xdecha.bot.on_callback_query(filters.regex(pattern=r"^pm_spam$"))
    async def pm_spam_callback(_, c_q: CallbackQuery):
        ...
        if c_q.from_user.id == owner.id:
            ...
            user_dict = await xdecha.get_user_dict(c_q.from_user.id)
            await c_q.edit_message_text(
                blocked_message.format_map(SafeDict(**user_dict)))
            await xdecha.block_user(c_q.from_user.id)
            await asyncio.sleep(1)
            await CHANNEL.log(
                f"#BLOCKED\n{c_q.from_user.mention} has been blocked due to spamming in pm !! ")

    @xdecha.bot.on_callback_query(filters.regex(pattern=r"^pm_contact$"))
