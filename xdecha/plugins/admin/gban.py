...
from xdecha import xdecha, Message, Config, get_collection, filters, pool
...
async def ungban_user(message: Message):
    ...
    if 'chat_ids' in found:
        for chat_id in found['chat_ids']:
            try:
                await xdecha.unban_chat_member(chat_id, user_id)
                await CHANNEL.log(
                    r"\\**#Antispam_Log**//"
                    f"\n**User:** [{firstname}](tg://user?id={user_id})\n"
                    f"**User ID:** `{user_id}`\n\n"
...
@xdecha.on_filters(filters.group & filters.new_chat_members, group=1, check_restrict_perm=True)
