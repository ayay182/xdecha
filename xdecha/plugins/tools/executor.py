...
from xdecha.utils import runcmd


@xdecha.on_cmd("eval", about={
    'header': "run python code line | lines",
    'flags': {'-s': "silent mode (hide STDIN)"},
    'usage': "{tr}eval [flag] [code lines]",
    'examples': [
        "{tr}eval print('xdecha')", "{tr}eval -s print('xdecha')",
...
async def eval_(message: Message):
    ...
    async def aexec(code):
        head = "async def __aexec(xdecha, message):\n "
        if '\n' in code:
        ...
        return await locals()['__aexec'](xdecha, message)
    try:
...
@xdecha.on_cmd("exec", about={
    'header': "run commands in exec",
    'usage': "{tr}exec [commands]",
    'examples': "{tr}exec echo \"xdecha\""}, allow_channels=False)
...
@xdecha.on_cmd("term", about={
    'header': "run commands in shell (terminal)",
    'usage': "{tr}term [commands]",
    'examples': "{tr}term echo \"xdecha\""}, allow_channels=False)
