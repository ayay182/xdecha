...
from xdecha.utils import get_import_path
from xdecha.plugins import ROOT


@xdecha.on_cmd("status", about={
...
async def status(message: Message) -> None:
    ...
    if not type_:
        out_str = f"""📊 **--xdecha Status--** 📊

🗃 **Plugins** : `{len(xdecha.manager.plugins)}`
        ✅ **Loaded** : `{len(xdecha.manager.loaded_plugins)}`
        ➕ **Enabled** : `{len(xdecha.manager.enabled_plugins)}`
        ➖ **Disabled** : `{len(xdecha.manager.disabled_plugins)}`
        ❎ **Unloaded** : `{len(xdecha.manager.unloaded_plugins)}`

⚔ **Commands** : `{len(xdecha.manager.commands)}`
        ✅ **Loaded** : `{len(xdecha.manager.loaded_commands)}`
        ➕ **Enabled** : `{len(xdecha.manager.enabled_commands)}`
        ➖ **Disabled** : `{len(xdecha.manager.disabled_commands)}`
        ❎ **Unloaded** : `{len(xdecha.manager.unloaded_commands)}`

⚖ **Filters** : `{len(xdecha.manager.filters)}`
        ✅ **Loaded** : `{len(xdecha.manager.loaded_filters)}`
        ➕ **Enabled** : `{len(xdecha.manager.enabled_filters)}`
        ➖ **Disabled** : `{len(xdecha.manager.disabled_filters)}`
        ❎ **Unloaded** : `{len(xdecha.manager.unloaded_filters)}`
"""
    elif 'p' in type_:
        if name_:
            if name_ in xdecha.manager.plugins:
                plg = xdecha.manager.plugins[name_]
...
🗂 **Total** : `{len(xdecha.manager.plugins)}`
✅ **Loaded** : `{len(xdecha.manager.loaded_plugins)}`
➕ **Enabled** : `{len(xdecha.manager.enabled_plugins)}`
➖ **Disabled** : `{len(xdecha.manager.disabled_plugins)}`
        `{'`,    `'.join((cmd.name for cmd in xdecha.manager.disabled_plugins))}`
❎ **Unloaded** : `{len(xdecha.manager.unloaded_plugins)}`
        `{'`,    `'.join((cmd.name for cmd in xdecha.manager.unloaded_plugins))}`
"""
    elif 'c' in type_:
        if name_:
            if not name_.startswith(Config.CMD_TRIGGER):
                n_name_ = Config.CMD_TRIGGER + name_
            if name_ in xdecha.manager.commands:
                cmd = xdecha.manager.commands[name_]
            elif n_name_ in xdecha.manager.commands:
                cmd = xdecha.manager.commands[n_name_]
            else:
...
🗂 **Total** : `{len(xdecha.manager.commands)}`
✅ **Loaded** : `{len(xdecha.manager.loaded_commands)}`
➕ **Enabled** : `{len(xdecha.manager.enabled_commands)}`
➖ **Disabled** : `{len(xdecha.manager.disabled_commands)}`
        `{'`,    `'.join((cmd.name for cmd in xdecha.manager.disabled_commands))}`
❎ **Unloaded** : `{len(xdecha.manager.unloaded_commands)}`
        `{'`,    `'.join((cmd.name for cmd in xdecha.manager.unloaded_commands))}`
"""
    elif 'f' in type_:
        if name_:
            if name_ in xdecha.manager.filters:
                flt = xdecha.manager.filters[name_]
...
🗂 **Total** : `{len(xdecha.manager.filters)}`
✅ **Loaded** : `{len(xdecha.manager.loaded_filters)}`
➕ **Enabled** : `{len(xdecha.manager.enabled_filters)}`
        `{'`,    `'.join((flt.name for flt in xdecha.manager.enabled_filters))}`
➖ **Disabled** : `{len(xdecha.manager.disabled_filters)}`
        `{'`,    `'.join((flt.name for flt in xdecha.manager.disabled_filters))}`
❎ **Unloaded** : `{len(xdecha.manager.unloaded_filters)}`
        `{'`,    `'.join((flt.name for flt in xdecha.manager.unloaded_filters))}`
"""
    else:
...
async def enable(message: Message) -> None:
    ...
    type_ = list(message.flags)
    if 'p' in type_:
        found = set(names_).intersection(set(xdecha.manager.plugins))
        if found:
            out = await xdecha.manager.enable_plugins(list(found))
            if out:
        ...
        found = set(names_).intersection(set(xdecha.manager.commands))
        if found:
            out = await xdecha.manager.enable_commands(list(found))
            if out:
        ...
        found = set(names_).intersection(set(xdecha.manager.filters))
        if found:
            out = await xdecha.manager.enable_filters(list(found))
            if out:
...
async def disable(message: Message) -> None:
    ...
    if 'p' in type_ and names_:
        found = set(names_).intersection(set(xdecha.manager.plugins))
        if found:
            out = await xdecha.manager.disable_plugins(list(found))
            if out:
        ...
        found = set(names_).intersection(set(xdecha.manager.commands))
        if found:
            out = await xdecha.manager.disable_commands(list(found))
            if out:
        ...
        found = set(names_).intersection(set(xdecha.manager.filters))
        if found:
            out = await xdecha.manager.disable_filters(list(found))
            if out:
...
async def load(message: Message) -> None:
    ...
    if message.flags:
        ...
        if 'p' in type_:
            found = set(names_).intersection(set(xdecha.manager.plugins))
            if found:
                out = await xdecha.manager.load_plugins(list(found))
                if out:
            ...
            found = set(names_).intersection(set(xdecha.manager.commands))
            if found:
                out = await xdecha.manager.load_commands(list(found))
                if out:
            ...
            found = set(names_).intersection(set(xdecha.manager.filters))
            if found:
                out = await xdecha.manager.load_filters(list(found))
                if out:
        ...
        if replied and replied.document:
            ...
            if file_.file_name.endswith('.py') and file_.file_size < 2 ** 20:
                ...
                try:
                    await xdecha.load_plugin(plugin, reload_plugin=True)
                    await xdecha.finalize_load()
...
async def unload(message: Message) -> None:
    ...
    if 'p' in type_ and names_:
        found = set(names_).intersection(set(xdecha.manager.plugins))
        if found:
            out = await xdecha.manager.unload_plugins(list(found))
            if out:
        ...
        found = set(names_).intersection(set(xdecha.manager.commands))
        if found:
            out = await xdecha.manager.unload_commands(list(found))
            if out:
        ...
        found = set(names_).intersection(set(xdecha.manager.filters))
        if found:
            out = await xdecha.manager.unload_filters(list(found))
            if out:
...
@xdecha.on_cmd('reload', about={'header': "Reload all plugins"}, allow_channels=False)
async def reload_(message: Message) -> None:
    """ Reload all plugins """
    await message.edit("`Reloading All Plugins`")
    await message.edit(
        f"`Reloaded {await xdecha.reload_plugins()} Plugins`", del_in=3, log=__name__)


@xdecha.on_cmd('clear', about={'header': "clear all save filters in DB"}, allow_channels=False)
async def clear_(message: Message) -> None:
    """ clear all save filters in DB """
    await message.edit("`Clearing DB...`")
    await message.edit(
        f"**Cleared Filters** : `{await xdecha.manager.clear()}`", del_in=3, log=__name__)
