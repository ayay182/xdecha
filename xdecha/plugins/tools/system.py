...
from xdecha.core.ext import RawClient
from xdecha import xdecha, Message, Config, get_collection
from xdecha.utils import terminate
...
async def restart_(message: Message):
    """ restart xdecha """
    await message.edit("Restarting xdecha Services", log=__name__)
    LOG.info("xdecha Services - Restart initiated")
    ...
    if Config.HEROKU_APP and 'h' in message.flags:
        ...
        asyncio.get_event_loop().create_task(xdecha.restart())


@xdecha.on_cmd("shutdown", about={'header': "shutdown xdecha :)"}, allow_channels=False)
...
async def die_(message: Message) -> None:
    """ set offline timeout to die xdecha """
    global MAX_IDLE_TIME  # pylint: disable=global-statement
...
@xdecha.on_cmd("listdisabled", about={'header': "List all disabled chats."})
...
@xdecha.on_cmd("sleep (\\d+)", about={
    'header': "sleep xdecha :P",
...
async def _slp_wrkr(seconds: int) -> None:
    ...
    await xdecha.reload_plugins()
    await xdecha.start()


@xdecha.on_user_status()
