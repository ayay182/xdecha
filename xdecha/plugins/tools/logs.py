from xdecha import xdecha, Message, logging, Config, pool
...
@xdecha.on_cmd("logs", about={
    'header': "check xdecha logs",
    'flags': {
        '-h': "get heroku logs",
...
async def check_logs(message: Message):
    ...
    if '-h' in message.flags and Config.HEROKU_APP:
        ...
        await message.client.send_as_file(chat_id=message.chat.id,
                                          text=logs,
                                          filename='xdecha-heroku.log',
                                          caption=f'xdecha-heroku.log [ {limit} lines ]')
    else:
        await message.client.send_document(chat_id=message.chat.id,
                                           document="logs/xdecha.log",
                                           caption='xdecha.log')
    await message.delete()


@xdecha.on_cmd("setlvl", about={
