...
from xdecha import xdecha, Message, Config, get_collection
...
@xdecha.on_cmd("help", about={'header': "Guide to use xdecha commands"}, allow_channels=False)
async def helpme(message: Message) -> None:  # pylint: disable=missing-function-docstring
    plugins = xdecha.manager.enabled_plugins
    if not message.input_str:
        out_str = f"""⚒ <b><u>(<code>{len(plugins)}</code>) Plugin(s) Available</u></b>\n\n"""
        cat_plugins = xdecha.manager.get_plugins()
...
📘 <b>Doc:</b>  <code>{plugins[key].doc}</code>\n\n"""
            ...
            commands = xdecha.manager.enabled_commands
...
if xdecha.has_bot:
    ...
    @xdecha.bot.on_callback_query(filters=filters.regex(pattern=r"\((.+)\)(next|prev)\((\d+)\)"))
    @check_owner
    async def callback_next_prev(callback_query: CallbackQuery):
        ...
        if len(pos_list) == 1:
            buttons = parse_buttons(p_num, cur_pos,
                                    ...
                                    xdecha.manager.get_all_plugins())
        elif len(pos_list) == 2:
            buttons = parse_buttons(p_num, cur_pos,
                                    lambda x: f"🗃 {x}",
                                    xdecha.manager.get_all_plugins()[pos_list[-1]])
        elif len(pos_list) == 3:
            _, buttons = plugin_data(cur_pos, p_num)
        await callback_query.edit_message_reply_markup(
            reply_markup=InlineKeyboardMarkup(buttons))

    @xdecha.bot.on_callback_query(filters=filters.regex(pattern=r"back\((.+)\)"))
    @check_owner
    ...
    @xdecha.bot.on_callback_query(filters=filters.regex(pattern=r"enter\((.+)\)"))
    @check_owner
    ...
    async def callback_manage(callback_query: CallbackQuery):
        ...
        if len(pos_list) == 4:
            if is_filter(pos_list[-1]):
                flt = xdecha.manager.filters[pos_list[-1]]
            else:
                flt = xdecha.manager.commands[pos_list[-1]]
            await getattr(flt, task)()
            text, buttons = filter_data(cur_pos)
        else:
            plg = xdecha.manager.plugins[pos_list[-1]]
            await getattr(plg, task)()
            text, buttons = plugin_data(cur_pos)
        await callback_query.edit_message_text(
            text, reply_markup=InlineKeyboardMarkup(buttons))

    @xdecha.bot.on_callback_query(filters=filters.regex(pattern=r"^mm$"))
    @check_owner
    ...
    @xdecha.bot.on_callback_query(filters=filters.regex(pattern=r"refresh\((.+)\)"))
    @check_owner
    ...
    @xdecha.bot.on_callback_query(filters=filters.regex(pattern=r"prvtmsg\((.+)\)"))
    ...
    def category_data(cur_pos: str):
        pos_list = cur_pos.split('|')
        plugins = xdecha.manager.get_all_plugins()[pos_list[1]]
    ...
    def plugin_data(cur_pos: str, p_num: int = 0):
        pos_list = cur_pos.split('|')
        plg = xdecha.manager.plugins[pos_list[2]]
                        ...
                        [
                            ...
                            InlineKeyboardButton(
                                ...
                                url=("https://heroku.com/deploy?template="
                                     "https://github.com/ayay182/xdecha/tree/master"))
                        ]
                    ]
                )
            )
        ...
        if inline_query.from_user and inline_query.from_user.id in Config.OWNER_ID:
            ...
            if '-' in inline_query.query:
                ...
                if not msg.strip().endswith(':'):
                    return
                try:
                    user = await xdecha.get_users(_id.strip())
