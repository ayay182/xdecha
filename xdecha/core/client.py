...
from xdecha import logging, Config, logbot
from xdecha.utils import time_formatter
from xdecha.utils.exceptions import xdechaBotNotFound
from xdecha.plugins import get_all_plugins
...
class _Abstractxdecha(Methods, RawClient):
    @property
    ...
    async def load_plugin(self, name: str, reload_plugin: bool = False) -> None:
        """ Load plugin to xdecha """
        _LOG.debug(_LOG_STR, f"Importing {name}")
        _IMPORTED.append(
            importlib.import_module(f"xdecha.plugins.{name}"))
        if reload_plugin:
...
class xdechaBot(_Abstractxdecha):
    """ xdechaBot, the bot """
    def __init__(self, **kwargs) -> None:
        _LOG.info(_LOG_STR, "Setting xdechaBot Configs")
        super().__init__(session_name=":memory:", **kwargs)

    @property
    def ubot(self) -> 'xdecha':
        """ returns userbot """
        return self._bot


class xdecha(_Abstractxdecha):
    """ xdecha, the userbot """

    has_bot = bool(Config.BOT_TOKEN)

    def __init__(self, **kwargs) -> None:
        _LOG.info(_LOG_STR, "Setting xdecha Configs")
        kwargs = {
        ...
        if Config.HU_STRING_SESSION and Config.BOT_TOKEN:
            ...
            kwargs['bot'] = xdechaBot(bot=self, **kwargs)
    ...
    @property
    def bot(self) -> Union['xdechaBot', 'xdecha']:
        """ returns xdechabot """
        if self._bot is None:
            if Config.BOT_TOKEN:
                return self
            raise xdechaBotNotFound("Need BOT_TOKEN ENV!")
        return self._bot

    async def start(self) -> None:
        """ start client and bot """
        _LOG.info(_LOG_STR, "Starting xdecha")
        await super().start()
        if self._bot is not None:
            _LOG.info(_LOG_STR, "Starting xdechaBot")
    ...
    async def stop(self) -> None:  # pylint: disable=arguments-differ
        """ stop client and bot """
        if self._bot is not None:
            _LOG.info(_LOG_STR, "Stopping xdechaBot")
            await self._bot.stop()
        _LOG.info(_LOG_STR, "Stopping xdecha")
        await super().stop()
        _close_db()
    ...
    def begin(self, coro: Optional[Awaitable[Any]] = None) -> None:
        ...
        async def _shutdown(sig: signal.Signals) -> None:
            _LOG.info(_LOG_STR, f"Received Stop Signal [{sig.name}], Exiting xdecha ...")
            await _finalize()
        ...
        logbot.edit_last_msg("xdecha has Started Successfully !")
        logbot.end()
        ...
        try:
            if coro:
                ...
                _LOG.info(_LOG_STR, f"Idling xdecha - {mode}")
                idle()
